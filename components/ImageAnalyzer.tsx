...
import { ImageSession, ChatMessage, Profile } from '../types';
import { getChatResponseStreamForImage, getSmartSuggestions } from '../services/geminiService';
...
const InputBar: React.FC<{ onSend: (text: string) => void; isLoading: boolean }> = ({ onSend, isLoading }) => {
  ...
  return (
    <div className="flex items-end gap-2 p-2 bg-slate-700/50 rounded-lg border border-slate-700 focus-within:border-purple-500 transition-colors">
      <textarea ref={textareaRef} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder="Ask a question about the image..." rows={1} className="flex-grow bg-transparent p-2 text-slate-200 resize-none focus:outline-none placeholder-slate-400 max-h-48" disabled={isLoading} />
...
const ImageAnalyzer: React.FC<ImageAnalyzerProps> = ({ session, profile, onSendMessage, onUpdateMessage, onAddSuggestions }) => {
  ...
  const handleSend = useCallback(async (text: string) => {
    ...
    try {
      const stream = await getChatResponseStreamForImage(session.imageBase64, session.mimeType, fullHistory, text);
      ...
      console.error("Error in image chat flow:", error);
  ...
  return (
    <div className="bg-slate-800 rounded-lg flex flex-col h-full">
      <div className="p-4 border-b border-slate-700">
        ...
        <p className="text-sm text-slate-400">Image Analysis</p>
      </div>
      <div className="flex-grow p-4 overflow-y-auto space-y-6">
        <div className="bg-slate-900/50 p-2 rounded-lg flex justify-center items-center">
            <img src={`data:${session.mimeType};base64,${session.imageBase64}`} alt={session.name} className="max-h-64 max-w-full rounded-md object-contain" />
        </div>
